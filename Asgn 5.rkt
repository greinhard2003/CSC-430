...
(define topenv (list
                ...
                (binding 'tostring (primV 'tostring))))
...
(define (invalidid? [id : Sexp] [invalids : (Listof Symbol)]) : Boolean
  (match invalids
    ['() #f]
    [(cons f r) (if (equal? id f) #t (invalidid? id r))]))

;returns true if any id is in the list of invalids

(define (checkids [ids : (Listof Sexp)] [invalids : (Listof Symbol)]) : Boolean
  (match ids
    ['() #f]
...
(define (parse [sexpr : Sexp]) : ExprC
  ...
  (match sexpr
    [(list (? list? params) '=> body)
         (cond
           [(checkids params invalids) (error 'invalidid "AAQZ : found invalid id, given ~e" params)]
           [(not (equal? (check-duplicates params) #f))
            (error 'dupparams "AAQZ : function contains duplicate params, given ~e" params)]
           [else (parse-lambda params body)])]
    [(? real? n) (numC n)]
    [(? symbol? s)
    ...
    [(list 'bind (list (? symbol? s) '= exps) ... body)
     (cond
       ...
       [(checkids (cast s (Listof Sexp)) invalids) (error 'invalidid "AAQZ : found invalid id, given ~e" s)]
...
(define (tostring [val : Value]) : String
  (match val
    [(numV v) (~a v)]
    [(strV v) v]
    [(boolV v) (if v "true" "false")]))

;takes in a list of values and combines them into one string
(define (appendall [vals : (Listof Value)] [str : String]) : String
  (match vals
    ['() str]
    [(cons f r) (appendall r (string-append str (tostring f)))]))
  ...
  (match op
    ...
    ['println (cond
                [(equal? (length args) 1)
                 ...
                 (cond
                   [(strV? a)
                    (displayln (strV-str a))
                    (boolV #t)]
                   [else (error 'badargs "AAQZ : Invalid argument for println, expected string, given: ~e" args)])]
                [else (error 'numargs "AAQZ : println expected 1 argument, given ~e" (length args))])]
    ['read-num (cond
                 [(equal? (length args) 0)
                  (display "> ")
                  (define input (read-line))
                  (if (eof-object? input)
                      (error 'invalidnum "AAQZ : Unexpected end of input for read-num")
                      (let ([num (string->number input)])
                        (if (and num (real? num))
                            (numV num)
                            (error 'invalidnum "AAQZ : Invalid input for read-num, expected real number"))))]
                 [else (error 'numargs "AAQZ : read-num expected 0 arguments, given ~e" (length args))])]
    ['read-str (cond
                 [(equal? (length args) 0)
                  (display "> ")
                  (define input (read-line))
                  (if (eof-object? input)
                      (error 'invalidstr "AAQZ : Unexpected end of input for read-str")
                      (strV input))]
                 [else (error 'numargs "AAQZ : read-str expected 0 arguments, given ~e" (length args))])]
    ['++
     (strV (appendall args ""))]
    ['seq (last args)]
    ['tostring
      (cond
        [(equal? (length args) 1)
         (define a (first args))
         (strV (tostring a))] ; Converts the value to string format
        [else (error 'numargs "AAQZ : tostring expected 1 argument, given ~e" (length args))])]
...
(check-equal? (tostring (numV 7)) "7")
(check-equal? (tostring (strV "test")) "test")
(check-equal? (tostring (boolV #t)) "true")
(check-equal? (tostring (boolV #f)) "false")
(check-equal? (appendall (list (numV 7) (numV 8) (strV "Hello")) "") "78Hello")

(check-exn (regexp (regexp-quote "numargs"))
           (lambda () (primsolve '< (list (numV 7)))))
(check-exn (regexp (regexp-quote "numargs"))
           (lambda () (primsolve '< (list (numV 7) (numV 3) (numV 5)))))
(check-exn (regexp (regexp-quote "badargs")) 
           (lambda () (primsolve '< (list (strV "string") (numV 3)))))
(check-exn (regexp (regexp-quote "badargs")) 
           (lambda () (primsolve '< (list (numV 7) (boolV #t)))))
(check-exn (regexp (regexp-quote "badargs")) 
           (lambda () (primsolve '< (list (strV "string") (boolV #t)))))

(check-equal? (primsolve '++ (list (numV 1) (strV " apple"))) (strV "1 apple"))
...
(check-equal? (checkids '(a b c) '(if => = bind)) #f)
(check-equal? (checkids '(a => c) '(if => = bind)) #t)
...
(check-exn (regexp (regexp-quote "if")) (lambda () (parse '(if true 1))))
...
(check-exn (regexp (regexp-quote "invalidid")) (lambda () (parse '(bind (=> = "") "World"))))
(check-exn (regexp (regexp-quote "invalidid")) (lambda () (parse '((=>) => "World"))))
...
(check-exn (regexp "println") (lambda () (primsolve 'println (list (numV 1)))))
(check-equal? (top-interp '(println "Hello, world!")) "true")
(check-exn (regexp (regexp-quote "numargs")) (lambda () (primsolve 'println (list (strV "Hello")
                                                                                  (strV "Extra argument")))))
...
;input a type other than a number for this
;(check-exn (regexp (regexp-quote "invalidnum")) (lambda () (primsolve 'read-num '())))
;call again with eof to check for failure
;(check-exn (regexp (regexp-quote "invalidnum")) (lambda () (primsolve 'read-num '())))
;this case should actually never execute since it wont enter the readnum block with extra arguments
;but it feels wrong not including it
#;(check-exn (regexp (regexp-quote "numargs"))
           (lambda () (primsolve 'read-num (list (numV 7)) '())))
;(primsolve 'read-str '())
;(top-interp '(++ "hello" (+ 4 3)))
;input eof for this one to check for an invalid string
;(check-exn (regexp (regexp-quote "invalidstr"))(lambda () (primsolve 'read-str)))
;use this to check for improper use of arguments in read-str
;(check-exn (regexp (regexp-quote "numargs"))(lambda () (primsolve 'read-str (list (numV 7))))
(check-exn (regexp (regexp-quote "numargs"))(lambda () (primsolve 'tostring (list (numV 7) (numV 9)))))
#;(top-interp '{seq
              {println "test"}
              {println "Pick a number."}
              {bind
               [num = {read-num}]
               {println {++ "You picked the number " num " correct?"}}}
              })
(define example-program
  '{seq
    {println "You walk into an old western bar and see 10 people surrounding the body of a dead man."}
    {println "Since you are the town sheriff its up to you to determine which of the suspects committed the crime."}
    {println "This is what you notice about each suspect."}
    {println "Suspect #1: Has red stains on the cuffs of his sleeves but is nursing a bowl of tomato soup."}
    {println (++ "Suspect #2: Appears to be unable to look at the victim,"
                 " he looks as if his meal is about to come back up.")}
    {println "Suspect #3: Clutching his revolver eyeing the other suspects nervously."}
    {println "Suspect #4: Slumped over the counter, barely able to keep his eyes open."}
    {println "Suspect #5: Eyes keep darting between #6 and #7, do you detect fear in his eyes?"}
    {println (++ "Suspect #6: Has been shouting incoherently at #6 since you walked in."
                 " Perhaps if he drank less he would be able to tell you the story.")}
    {println "Suspect #7: Smells faintly of gunpowder and has a wild look in his eyes."}
    {println "Suspect #8: Has been sobbing uncontrolably into the palms of his hands, he's not built for this world."}
    {println (++ "Suspect #9: His eyes are full of hate,"
                 " is this because he commit the crime or because you locked up his brother?")}
    {println "Suspect #10: Appeared to be unaware of what had just occurred behind him, is he telling you the truth?"}
    {println "You have 3 attempts to guess the criminal before you loose your badge."}
   
    ; Initialize game variables
    {bind
     [target-number = 7]     ; The number to guess
     [tries-left = 3]
     
     
     ; Define the game loop, with target-number passed as a parameter
     [guess-loop = {(target tries self)
                    =>
                    {seq
                      {println {++ "You have " (tostring tries) " attempts remaining."}}
                     
                      {if {<= tries 0}
                          {println (++ "You failed to guess who the criminal was,"
                                       " #" (tostring target)
                                       " walks free and you lose your badge, doomed to become a sad broken old man")}
                          ; Final output if tries are exhausted
                          
                          ; If tries remain, prompt for a guess
                          {seq
                            {println {++ "Tries left: " (tostring tries) ". Who do you accuse? (pick a number 1-10): "}}
                            {bind [guess = {read-num}]
                              {seq
                                ; Check the guess
                                {if {equal? guess target}      
                                      {println (++ "Congratulations! You correctly accused the murderer!\n"
                                                   "However in your effort to make your decision, it appears"
                                                   " you failed to notice him sneak out the back.\n"
                                                   "How could you have let this happen?"
                                                   " There's no doubt you'll lose your badge"
                                                   " once the mayor finds out about this.")}
                                    {seq
                                      {println "How dare you accuse an innocent man of murder?!"}
                                      ; Recursive call with one less try if the guess is incorrect
                                      (self target {- tries 1} self)
                                    }
                                }
                              }}
                          }
     ...
     (guess-loop target-number tries-left guess-loop)
    }
  }
)


; Execute the example program by passing it to the top-level interpreter function
